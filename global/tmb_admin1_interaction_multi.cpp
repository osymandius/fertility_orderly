  ...
  
{
  ...
  // DATA_MATRIX(X_tips_dummy_interaction);
  PARAMETER_VECTOR(beta_tips_dummy);
  // PARAMETER_VECTOR(beta_tips_dummy_interaction);
  ...
  DATA_SPARSE_MATRIX(R_spatial_interaction);
  ...
  DATA_SPARSE_MATRIX(Z_interaction4);
  // PARAMETER_ARRAY(eta4);
  // PARAMETER(log_prec_eta4);
  // PARAMETER(lag_logit_eta4_phi_tips);
  // DATA_SPARSE_MATRIX(R_survey); //eta4
  ...
  // nll -= dnorm(beta_tips_dummy_interaction, Type(0), Type(sqrt(1/0.001)), true).sum();
  ...
  nll -= dnorm(u_tips.sum(), Type(0), Type(0.01) * u_tips.size(), true);
   ...
   // ETA-4 - TIPS x Survey interaction

  // nll -= dlgamma(log_prec_eta4, Type(1), Type(20000), true);
  // Type prec_eta4 = exp(log_prec_eta4);

  // nll -= dnorm(lag_logit_eta4_phi_tips, Type(0), Type(sqrt(1/0.15)), true);
  // Type eta4_phi_tips = 2*exp(lag_logit_eta4_phi_tips)/(1+exp(lag_logit_eta4_phi_tips))-1;
  
  // nll += SEPARABLE(AR1(Type(eta4_phi_tips)), GMRF(R_survey))(eta4);
  // vector<Type> eta4_v(eta4);
  ...
  DATA_MATRIX(X_period_interaction);
  PARAMETER(lag_logit_phi_arima_period);

  PARAMETER_VECTOR(beta_period);
  nll -= dnorm(beta_period, Type(0), Type(sqrt(1/0.001)), true).sum();

  PARAMETER_VECTOR(beta_period_interaction);
  nll -= dnorm(beta_period_interaction, Type(0), Type(sqrt(1/0.001)), true).sum();
  nll -= dnorm(beta_period_interaction.sum(), Type(0), Type(0.01) * beta_period_interaction.size(), 1);
  ...
  nll += SEPARABLE(AR1(Type(eta2_phi_period)), GMRF(R_spatial_interaction))(eta2);
  // nll += SEPARABLE(GMRF(R_period_iid), GMRF(R_spatial))(eta2);
  ...
  nll += SEPARABLE(AR1(Type(eta3_phi_age)), GMRF(R_spatial_interaction))(eta3);
  ...
  vector<Type> log_lambda(
                     ...
                     + X_period_interaction * beta_period_interaction
                     // + Z_spatial * spatial                     
  ...
  vector<Type> mu_obs_pred_dhs(X_extract_dhs * (M_full_obs * log(lambda_out))
                                ...
                                // + X_tips_dummy_interaction * beta_tips_dummy_interaction          // TIPS fixed effect
  ...
  REPORT(beta_period);
  REPORT(beta_period_interaction);
  REPORT(lag_logit_phi_arima_period);

  REPORT(beta_tips_dummy);
  // REPORT(beta_tips_dummy_interaction);
  REPORT(u_tips);
