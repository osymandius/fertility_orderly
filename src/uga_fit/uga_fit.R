...
population <- read.csv(paste0("depends/", tolower(iso3), "_population_ubos.csv"))
...
R_smooth_iid <- .sparseDiagonal(length(unique(mf$observations$full_obs$id.smooth)))
# iid <- Diagonal(length(unique(mf$observations$full_obs$id.smooth)), kind="d")
...
# 
# tmb_results_nb <- dfertility::tmb_outputs(fit, mf, areas) %>%
#   mutate(source = "NB")

# TMB::compile("global/tmb_all_level_iid.cpp")               # Compile the C++ file
# dyn.load(dynlib("global/tmb_all_level_iid"))

TMB::compile("resources/tmb_all_level_iid.cpp")               # Compile the C++ file
dyn.load(dynlib("resources/tmb_all_level_iid"))
...
if(mf$mics_toggle) {
  tmb_int$data <- c(tmb_int$data,
                    "Z_tips_mics" = mf$Z$Z_tips_mics,
                    "R_tips_mics" = mf$R$R_tips_mics,
                    "Z_smooth_iid_mics" = Z_smooth_iid_mics,
                    "log_offset_mics" = list(log(filter(mf$observations$full_obs, survtype == "MICS")$pys)),
                    "births_obs_mics" = list(filter(mf$observations$full_obs, survtype == "MICS")$births),
...
tmb_results_nb_smooth <- dfertility::tmb_outputs(fit, mf, areas) %>%
  mutate(source = "NB smooth")

# TMB::compile("global/tmb_all_level_iid_poisson.cpp")               # Compile the C++ file
# dyn.load(dynlib("global/tmb_all_level_iid_poisson"))

TMB::compile("resources/tmb_all_level_iid_poisson.cpp")               # Compile the C++ file
dyn.load(dynlib("resources/tmb_all_level_iid_poisson"))
...
tmb_results_poisson_smooth <- dfertility::tmb_outputs(fit, mf, areas) %>%
  mutate(source = "Poisson smooth")

tmb_results <- tmb_results_nb_smooth %>% bind_rows(tmb_results_poisson_smooth)
