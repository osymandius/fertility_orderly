...
clusters <- read.csv("depends/cod_dhs_clusters.csv")
...
write_csv(asfr, "cod_dhs_asfr.csv")



mics_wm <- read.csv("depends/cod_mics_women.csv") %>%
  # mics_wm <- read.csv("archive/cod_survey/20201125-172927-c92fe668/cod_mics_women.csv") %>%
  filter(survey_id != "COD2010MICS") %>%
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()

mics_births_to_women <- read.csv("depends/cod_mics_births_to_women.csv") %>%
  # mics_births_to_women <- read.csv("archive/cod_survey/20201125-172927-c92fe668/cod_mics_births_to_women.csv") %>%
  filter(survey_id != "COD2010MICS") %>%
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()
...
write_csv(mics_asfr, "cod_mics_asfr.csv")

asfr <- asfr %>%
  bind_rows(mics_asfr)

write_csv(asfr, "cod_asfr.csv")