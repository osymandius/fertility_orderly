...
clusters <- read.csv("depends/civ_dhs_clusters.csv")
...
write_csv(asfr, "civ_dhs_asfr.csv")



mics_wm <- read.csv("depends/civ_mics_women.csv") %>%
  # mics_wm <- read.csv("archive/civ_survey/20201125-175553-8280efac/civ_mics_women.csv") %>%
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()

mics_births_to_women <- read.csv("depends/civ_mics_births_to_women.csv") %>%
  # mics_births_to_women <- read.csv("archive/civ_survey/20201125-175553-8280efac/civ_mics_births_to_women.csv") %>%
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()
...
write_csv(mics_asfr, "civ_mics_asfr.csv")

asfr <- asfr %>%
  bind_rows(mics_asfr)

write_csv(asfr, "civ_asfr.csv")