...
clusters <- read.csv("depends/swz_dhs_clusters.csv")
...
write_csv(asfr, "swz_dhs_asfr.csv")



mics_wm <- read.csv("depends/swz_mics_women.csv") %>%
  # mics_wm <- read.csv("draft/swz_survey/20201123-204826-542293e7/swz_mics_women.csv") %>%
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()

mics_births_to_women <- read.csv("depends/swz_mics_births_to_women.csv") %>%
  # mics_births_to_women <- read.csv("draft/swz_survey/20201123-204826-542293e7/swz_mics_births_to_women.csv") %>% 
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()
...
write_csv(mics_asfr, "swz_mics_asfr.csv")

asfr <- asfr %>%
  bind_rows(mics_asfr)

write_csv(asfr, "swz_asfr.csv")