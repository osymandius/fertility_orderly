areas <- read_sf("depends/nga_areas.geojson")
clusters <- read.csv("depends/nga_dhs_clusters.csv")
...
write_csv(asfr, "nga_dhs_asfr.csv")



mics_wm <- read.csv("depends/nga_mics_women.csv") %>%
  # mics_wm <- read.csv("draft/nga_survey/20201123-204826-542293e7/nga_mics_women.csv") %>%
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()

mics_births_to_women <- read.csv("depends/nga_mics_births_to_women.csv") %>%
  # mics_births_to_women <- read.csv("draft/nga_survey/20201123-204826-542293e7/nga_mics_births_to_women.csv") %>% 
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()
...
write_csv(mics_asfr, "nga_mics_asfr.csv")

asfr <- asfr %>%
  bind_rows(mics_asfr)

write_csv(asfr, "nga_asfr.csv")