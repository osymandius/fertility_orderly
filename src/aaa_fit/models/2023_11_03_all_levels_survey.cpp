...

{
  ...
  if(multiple_survey_toggle && !mics_only_toggle) {
    ...
    nll -= dnorm(u_survey.sum(), Type(0), Type(0.01) * u_survey.size(), true);
    
    survey_lh = Z_survey  * u_survey * sqrt(1/prec_survey);
    ...
    mu_obs_pred_phia = mu_obs_pred_phia
      + X_extract_phia * survey_lh;
