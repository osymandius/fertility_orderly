...
population <- read.csv("archive/zmb_data_population/20201130-163948-1fa3fdee/zmb_population_nso.csv")
areas <- read_sf("archive/zmb_data_areas/20201029-105226-c8af5982/zmb_areas.geojson")
asfr <- read.csv("archive/zmb_asfr/20201203-174916-768b9c38/zmb_dhs_asfr.csv")
# asfr <- readRDS("~/Documents/GitHub/subnat_fertility/countries/ZMB/data/ZMB_asfr_admin1.rds")

# asfr <- asfr %>%
#   convert_age_groups() %>%
#   filter(!is.na(surveyid)) %>%
#   rename(survey_id = surveyid) %>%
#   select(area_id, period, age_group, pys:births)
...
tmb_results %>%
  filter(area_level == 1, variable == "tfr", str_detect(area_id, "ZMB")) %>%
...
fr_plot <- read.csv("archive/zmb_asfr/20201203-174916-768b9c38/zmb_fr_plot.csv") %>%
