...
clusters <- read.csv("depends/bdi_dhs_clusters.csv")
...
write_csv(asfr, "bdi_dhs_asfr.csv")



mics_wm <- read.csv("depends/bdi_mics_women.csv") %>%
  # mics_wm <- read.csv("archive/bdi_survey/20201125-175553-8280efac/bdi_mics_women.csv") %>%
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()

mics_births_to_women <- read.csv("depends/bdi_mics_births_to_women.csv") %>%
  # mics_births_to_women <- read.csv("archive/bdi_survey/20201125-175553-8280efac/bdi_mics_births_to_women.csv") %>%
  arrange(survey_id) %>%
  group_by(survey_id) %>%
  group_split()
...
write_csv(mics_asfr, "bdi_mics_asfr.csv")

asfr <- asfr %>%
  bind_rows(mics_asfr)

write_csv(asfr, "bdi_asfr.csv")